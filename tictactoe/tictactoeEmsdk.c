...
#include <time.h>
#include <stdbool.h>
#include <C:\emsdk\upstream\emscripten\cache\sysroot\include\emscripten\emscripten.h>

char str[] = "TicTacToe\n\nPress the key corresponding to the relevant square on the grid to make your move.\n\n 7 | 8 | 9 \n-----------\n 4 | 5 | 6 \n-----------\n 1 | 2 | 3 \n\nYou are playing as  \n\n";
int numpadLookup[] = {6, 7, 8, 3, 4, 5, 0, 1, 2};
...
void play()
{
    ...
    int move = getBestMove(board, computer);
    board[move] = computer;
    played();
...
EMSCRIPTEN_KEEPALIVE void input(char input)
{
    if (ended || turn != player)
    {
        return;
    }
    printf("%c\n", input);
